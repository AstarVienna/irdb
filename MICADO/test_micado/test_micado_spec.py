...
class TestInit:
    ...
    def test_runs_spec_hk_15000x20(self):
        ...
        cmds_img = sim.UserCommands(
            use_instrument="MICADO", set_modes=["SCAO", "IMG_4mas"]
        )
        ...
        cmds = sim.UserCommands(
            use_instrument="MICADO", set_modes=["SCAO", "SPEC_15000x20"]
        )
        ...
        if PLOTS:
            plt.subplot(121)
            plt.imshow(micado.image_planes[0].data, norm="log", origin="lower")
            plt.subplot(122)
            plt.imshow(hdul[1].data, norm="log", origin="lower")
