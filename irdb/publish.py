...
def _get_local_path(pkg_name: str, stable: bool) -> Path:
    ...
    try:
        ...
        raise ValueError(
            f"No compiled version of '{pkg_name}' found for "
            f"condition '{stable=}'."
        ) from err
    return local_path
...
def _handle_missing_folder(pkg_name: str):
    ...
    proceed = input(
        "Do you want to add a server folder now? Upload will be aborted "
        f"otherwise. Also check spelling for '{pkg_name}' before proceeding!"
        "    (y)/n:  "
    )
    if not proceed.lower() == "y":
        raise KeyboardInterrupt("Execution aborted by user.")

    allowed = {"locations", "telescopes", "instruments"}
    # TODO: py311 py312 Remove silly triple quotes...
    new_folder = input(
        f"""Allowed values for server folder are: '{"', '".join(allowed)}':  """
    )
...
def _get_server_path(pkg_name: str, local_name: str) -> str:
    with PATH_FOLDERS_YAML.open("r", encoding="utf-8") as file:
        folders = yaml.safe_load(file)
    try:
        folder = folders[pkg_name]
    except KeyError:
        folder = _handle_missing_folder(pkg_name)[pkg_name]
    return f"{folder}/{local_name}"
...
def confirm(pkg_name: str) -> bool:
    ...
    proceed = input(
        f"This will supersede the current STABLE version ({current_stable}) of"
        f" '{pkg_name}' on the IRDB server. The uploaded package will be set "
        f"as the new default for '{pkg_name}'.\nAre you sure you want to "
        "continue?    (y)/n:  "
    )
...
def push_to_server(
    ...
    if (
        not local_path.stem.endswith("dev")
        and not no_confirm
        and not confirm(pkg_name)
    ):
        return
...
def main():
    ...
    parser = argparse.ArgumentParser(
        prog="publish",
        description=(
            "Set a new version number, compile (zip) the specified packages "
            "and (optionally) push them to the IRDB server. This command must "
            "be run from the IRDB root directory."
        ),
    )

    parser.add_argument(
        "pkg_names", nargs="+", help="Name(s) of the package(s)."
    )
    parser.add_argument(
        "-l",
        dest="username",
    ...
    parser.add_argument(
        ...
        help=(
            "UniVie u:space password. If left empty, a secure prompt will "
            "appear, which is the recommended usage. Supplying the password "
            "directly via this argument is unsecure and only included for "
            "script support."
        ),
    )
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Compile all files in a PKG folder to a .zip archive. If not set, "
            "the latest comiled version will be used (error if none is found)."
        ),
    )
    parser.add_argument(
        "-u",
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Build as a stable version. By default, a dev version is created. "
            "Publishing a stable version requires to set this option, and a "
            "manual conformation will be asked from the user."
        ),
    )
    parser.add_argument(
        "-k",
        ...
        action="store_true",
        help=(
            "Keep the current package version number as in the package's "
            "version.yaml file. By default, running this script will bump the "
            "package version number (date) and timestamp. Set this option to "
            "prevent that."
        ),
    )
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Don't ask for confirmation when uploading stable package. Only "
            "for CI/CD use!"
        ),
    )
    ...
    if not args.compile and not args.upload:
        logging.warning(
            "Neither `compile` nor `upload` option was set. "
